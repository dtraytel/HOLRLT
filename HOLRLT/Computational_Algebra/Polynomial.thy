...
imports
  "Complex_MainRLT"
  "HOLRLT-Library.More_List"
  "HOLRLT-Library.Infinite_Set"
  Factorial_Ring
begin
...
(****************)
(* The type poly has a wide definition *)

definition "isPoly (f :: nat \<Rightarrow> 'a::zero) \<equiv> \<forall>\<^sub>\<infinity> n. f n = 0"

lemma isPoly_aux: "isPoly = isPoly" ..
local_setup \<open>RLTHM @{binding isPoly_rlt_neper_param} @{thm isPoly_aux}\<close>
thm isPoly_rlt_neper_param

lemma isPoly_def2: "isPoly f \<longleftrightarrow> finite {n. f n \<noteq> 0}"
unfolding isPoly_def eventually_cofinite ..

local_setup \<open>RLTHM @{binding isPoly_rlt_def2} @{thm isPoly_def2}\<close>
thm isPoly_rlt_def2[no_vars]

lemma isPoly_rlt_simp[simp]: "neper R \<Longrightarrow> R z z \<Longrightarrow> rel_fun (=) R f f \<Longrightarrow>
  isPoly_rlt R z f \<longleftrightarrow> finite {n. \<not> R (f n) z}"
using isPoly_rlt_def2 unfolding rlt_eq .

(* *)
lemmas poly_type_definition = 
poly.type_definition_poly[unfolded isPoly_def[symmetric]]

(* *)

definition addp :: "('a::zero \<Rightarrow> 'a \<Rightarrow> bool) \<Rightarrow> 'a \<Rightarrow> ('a \<Rightarrow> 'a \<Rightarrow> bool)"
where
"addp R z \<equiv> \<lambda>x y. R x y \<or> (\<not> R 0 0 \<and> (x = 0 \<and> y = 0 \<or> x = 0 \<and> R z y \<or> y = 0 \<and> R z x))"

lemma neper_addp[simp]: 
"neper R \<Longrightarrow> neper (addp R z)"
unfolding neper_def per_def addp_def by blast

definition rel_poly :: 
"('a::zero \<Rightarrow> 'a \<Rightarrow> bool) \<Rightarrow> 'a \<Rightarrow> 'a poly \<Rightarrow> 'a poly \<Rightarrow> bool" 
where 
"rel_poly R z = inv_imagep (rel_fun (=) (addp R z)) coeff"

lemma rel_poly_neper[simp]: "neper R \<Longrightarrow> R z z \<Longrightarrow> neper (rel_poly R z)"
unfolding rel_poly_def 
apply(rule neper_inv_imagep[of _ coeff "Abs_poly (\<lambda>_. 0)"])
  subgoal by auto
  subgoal apply(subst Abs_poly_inverse)
    unfolding perOfSet_def rel_fun_def addp_def 
    by (auto simp add: Abs_poly_inverse) .

lemma addp_eq[simp]: "addp (=) 0 = (=)"
unfolding addp_def by auto

lemma rel_poly_eq[simp]: "rel_poly (=) 0 = (=)"
unfolding rel_poly_def inv_imagep_def fun_eq_iff 
	by (simp add: coeff_inject)

(* *)

hide_const gg
definition gg :: 
"('a::zero \<Rightarrow> 'a \<Rightarrow> bool) \<Rightarrow> 'a \<Rightarrow> (nat \<Rightarrow> 'a) \<Rightarrow> (nat \<Rightarrow> 'a)" where 
"gg R z \<equiv> \<lambda>f i. 
  if f i = 0 \<or> R (f i) 0 then z 
  else if R (f i) z \<and> R 0 0 then 0
  else f i"

(* *)

lemma isPoly_rlt_gg: 
assumes R: "neper R" and z: "R z z" 
and f: "isPoly f" "rel_fun (=) (addp R z) f f"
shows "isPoly_rlt R z (gg R z f)" 
using assms apply(subst isPoly_rlt_simp)
  subgoal by meson
  subgoal by meson
  subgoal unfolding gg_def rel_fun_def addp_def  
  by auto
  subgoal 
  by (smt (verit, best) Ball_Collect gg_def  isPoly_def2 
        mem_Collect_eq rev_finite_subset z) .

lemma rel_fun_gg: 
assumes R: "neper R" and z: "R z z" 
and f: "isPoly f" "rel_fun (=) (addp R z) f f"
shows "rel_fun (=) R (gg R z f) (gg R z f)" 
using assms unfolding rel_fun_def gg_def addp_def by auto
    
lemma gg_eq[simp]: "gg (=) 0 = id"
...
shows 
"bij_upto 
  (restr (rel_fun (=) (addp R z)) isPoly)
  (restr (rel_fun (=) R) (isPoly_rlt R z))  
  (gg R z)"
proof(rule bij_uptoI) 
  show "neper (restr (rel_fun (=) R) (isPoly_rlt R z))"
  apply(rule neper_restr)
    subgoal by (simp add: R option.rrel_neper)
    subgoal apply(rule exI[of _ "\<lambda>_.z"])
    using R z apply(subst isPoly_rlt_simp) unfolding rel_fun_def by auto .
next
  fix f f' 
  assume "restr (rel_fun (=) (addp R z)) isPoly f f'"
  hence ff': "isPoly f" "isPoly f'" "rel_fun (=) (addp R z) f f'"
   unfolding restr_def perOfSet_def by auto
  show "restr (rel_fun (=) R) (isPoly_rlt R z) (gg R z f) (gg R z f')" 
  unfolding restr_def proof safe
    show "rel_fun (=) R (gg R z f) (gg R z f')" 
    using z R using ff'(2,3) unfolding ff'(1) unfolding rel_fun_def gg_def addp_def 
    apply(cases "R 0 0")
      subgoal by simp (metis neper_def per_def)  
      subgoal by simp (metis neper_per per_def) .
    
    show "isPoly_rlt R z (gg R z f)" 
    by (metis R ff'(1) ff'(3) isPoly_rlt_gg neper_addp neper_eq neper_per 
     neper_rel_fun per_def z)
   
    show "isPoly_rlt R z (gg R z f')"
    by (metis R ff'(2) ff'(3) isPoly_rlt_gg neper_addp neper_eq neper_per 
     neper_rel_fun per_def z)
  qed
next
  fix f f'
  assume "restr (rel_fun (=) (addp R z)) isPoly f f"
  "restr (rel_fun (=) (addp R z)) isPoly f' f'"
  "restr (rel_fun (=) R) (isPoly_rlt R z) (gg R z f) (gg R z f')"

  hence ff: "isPoly f" "isPoly f'" "rel_fun (=) (addp R z) f f" "rel_fun (=) (addp R z) f' f'"
  and gg: "isPoly_rlt R z (gg R z f)" "isPoly_rlt R z (gg R z f')"
   "rel_fun (=) R (gg R z f) (gg R z f')"
  unfolding restr_def perOfSet_def by auto

  show "restr (rel_fun (=) (addp R z)) isPoly f f'"
  unfolding restr_def perOfSet_def proof safe
    show "rel_fun (=) (addp R z) f f'"
    using R z ff(3,4) gg(3) unfolding rel_fun_def unfolding gg_def addp_def  
    apply(cases "R 0 0")
      subgoal by simp (smt (verit, best) neper_per per_def)
      subgoal by simp (smt (verit, best) neper_per per_def) .        
    show "isPoly f" by fact
    show "isPoly f'" by fact
  qed
next
  fix g  
  assume "restr (rel_fun (=) R) (isPoly_rlt R z) g g"
  hence "isPoly_rlt R z g" and g: "rel_fun (=) R g g"
  unfolding restr_def by auto
  hence fin: "finite {n. \<not> R (g n) z}"   
  using R z by simp 
  hence ex: "\<exists>n. R (g n) z" 
  	using infinite_nat_iff_unbounded_le by auto

  define i0 where "i0 = (SOME i. R (g i) z)"
  have Ri0: "R (g i0) z"  
  	by (metis ex i0_def tfl_some)

  have fin': "finite (insert i0 {n. \<not> R (g n) z})"
   using fin by force

  define f where "f \<equiv> \<lambda>i. 
      if R 0 0 then (if R (g i) z then 0 else if R (g i) 0 then z else g i)
      else (if R (g i) z \<and> i \<noteq> i0 then 0 else g i)" 

  show "\<exists>f. restr (rel_fun (=) (addp R z)) isPoly f f \<and> 
            restr (rel_fun (=) R) (isPoly_rlt R z) g (gg R z f)"  
  proof(rule exI[of _ f], unfold restr_def, safe)
    have "{n. f n \<noteq> 0} \<subseteq> insert i0 {n. \<not> R (g n) z}"   
    unfolding f_def apply safe apply(cases "R 0 0") by auto
    thus f: "isPoly f" "isPoly f" unfolding isPoly_def2  
    	using fin' rev_finite_subset by blast+
    show "rel_fun (=) R g (gg R z f)"
    unfolding rel_fun_def gg_def f_def   
    by simp (smt (verit, best) R Ri0 g mem_Collect_eq neper_classes_eq rel_fun_def)

    show "isPoly_rlt R z g" by fact

    show f': "rel_fun (=) (addp R z) f f"
    using R z g unfolding rel_fun_def f_def addp_def by auto

    show "isPoly_rlt R z (gg R z f)"
    using isPoly_rlt_gg[OF R z f(1) f'] .  
  qed
qed 
...
wide_typedef poly rel: rel_poly rep: "\<lambda>R1 R2. gg R1 R2 \<circ> coeff"
...
(*
thm Abs_poly_qdef[THEN fun_cong]
term zero_poly_inst.zero_poly
thm zero_poly_def
ML \<open>Relativization.definition_of @{context} (@{const_name zero_poly_inst.zero_poly}, @{typ "'a poly"})\<close>
ML \<open>Sign.the_const_constraint @{theory} @{const_name zero_poly_inst.zero_poly}\<close>
ML \<open>Sign.the_const_type @{theory} @{const_name zero_poly_inst.zero_poly}\<close>
ML \<open>Sign.the_const_constraint @{theory} @{const_name Abs_poly}\<close>
ML \<open>Relativization.quasidef_of @{context} @{const_name Abs_poly}\<close>
local_setup\<open>RLTHM @{binding zero_poly_def} @{thm zero_poly_def}\<close>
local_setup\<open>RLTHM @{binding poly_eq_0_iff_dvd_rlt} @{thm poly_eq_0_iff_dvd}\<close>
*)
