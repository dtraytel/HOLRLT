...
(****************)
(* The type fls has a wide definition *)

definition "isFls (f :: int \<Rightarrow> 'a::zero) \<equiv> \<forall>\<^sub>\<infinity> n::nat. f (- int n) = 0"

lemma isFls_aux: "isFls = isFls" ..
local_setup \<open>RLTHM @{binding isFls_rlt_neper_param} @{thm isFls_aux}\<close>
thm isFls_rlt_neper_param

lemma isFls_def2: "isFls f \<longleftrightarrow> finite {i. i \<le> 0 \<and> f i \<noteq> 0}"
unfolding isFls_def eventually_cofinite 
 apply(rule bij_betw_finite[of "\<lambda>n. - int n"])
 unfolding bij_betw_def inj_on_def image_def 
 by auto (metis nonpos_int_cases)

local_setup \<open>RLTHM @{binding isFls_rlt_def2} @{thm isFls_def2}\<close>
thm isFls_rlt_def2[no_vars]

lemma isFls_rlt_simp[simp]: "neper R \<Longrightarrow> R z z \<Longrightarrow> rel_fun (=) R f f \<Longrightarrow>
  isFls_rlt R z f \<longleftrightarrow> finite {i. i \<le> 0 \<and> \<not> R (f i) z}"
using isFls_rlt_def2 unfolding rlt_eq .

(* *)
lemmas fls_type_definition = 
fls.type_definition_fls[unfolded isFls_def[symmetric]]
...
definition rel_fls :: 
"('a::zero \<Rightarrow> 'a \<Rightarrow> bool) \<Rightarrow> 'a \<Rightarrow> 'a fls \<Rightarrow> 'a fls \<Rightarrow> bool" 
where 
"rel_fls R z = inv_imagep (rel_fun (=) (addp R z)) fls_nth"

lemma rel_fls_neper[simp]: "neper R \<Longrightarrow> R z z \<Longrightarrow> neper (rel_fls R z)"
unfolding rel_fls_def 
apply(rule neper_inv_imagep[of _ fls_nth "Abs_fls (\<lambda>_. 0)"])
  subgoal by auto
  subgoal apply(subst Abs_fls_inverse)
    unfolding perOfSet_def rel_fun_def addp_def 
    by (auto simp add: Abs_fls_inverse) .

lemma addp_eq[simp]: "addp (=) 0 = (=)"
unfolding addp_def by auto

lemma rel_fls_eq[simp]: "rel_fls (=) 0 = (=)"
unfolding rel_fls_def inv_imagep_def fun_eq_iff 
	by (simp add: fls_nth_inject)

(* *)

hide_const gg
definition gg :: 
"('a::zero \<Rightarrow> 'a \<Rightarrow> bool) \<Rightarrow> 'a \<Rightarrow> (int \<Rightarrow> 'a) \<Rightarrow> (int \<Rightarrow> 'a)" where 
...
and f: "isFls f" "rel_fun (=) (addp R z) f f"
shows "isFls_rlt R z (gg R z f)" 
using assms apply(subst isFls_rlt_simp)
  subgoal by meson
  subgoal by meson
  subgoal unfolding gg_def rel_fun_def addp_def isFls_def2
  by auto
  subgoal 
  by (smt (verit, best) Ball_Collect gg_def  isFls_def2 
...
shows 
"bij_upto 
  (restr (rel_fun (=) (addp R z)) isFls)
  (restr (rel_fun (=) R) (isFls_rlt R z))  
  (gg R z)"
proof(rule bij_uptoI) 
  show "neper (restr (rel_fun (=) R) (isFls_rlt R z))"
  apply(rule neper_restr)
    subgoal by (simp add: R option.rrel_neper)
    subgoal apply(rule exI[of _ "\<lambda>_.z"])
    using R z apply(subst isFls_rlt_simp) unfolding rel_fun_def by auto .
next
  fix f f' 
  assume "restr (rel_fun (=) (addp R z)) isFls f f'"
  hence ff': "isFls f" "isFls f'" "rel_fun (=) (addp R z) f f'"
   unfolding restr_def perOfSet_def by auto
  show "restr (rel_fun (=) R) (isFls_rlt R z) (gg R z f) (gg R z f')" 
  unfolding restr_def proof safe
    ...
    show "isFls_rlt R z (gg R z f)" 
    by (metis R ff'(1) ff'(3) isFls_rlt_gg neper_addp neper_eq neper_per 
     neper_rel_fun per_def z)
   
    show "isFls_rlt R z (gg R z f')"
    by (metis R ff'(2) ff'(3) isFls_rlt_gg neper_addp neper_eq neper_per 
     neper_rel_fun per_def z)
  qed
next
  fix f f'
  assume "restr (rel_fun (=) (addp R z)) isFls f f"
  "restr (rel_fun (=) (addp R z)) isFls f' f'"
  "restr (rel_fun (=) R) (isFls_rlt R z) (gg R z f) (gg R z f')"

  hence ff: "isFls f" "isFls f'" "rel_fun (=) (addp R z) f f" "rel_fun (=) (addp R z) f' f'"
  and gg: "isFls_rlt R z (gg R z f)" "isFls_rlt R z (gg R z f')"
   "rel_fun (=) R (gg R z f) (gg R z f')"
  unfolding restr_def perOfSet_def by auto

  show "restr (rel_fun (=) (addp R z)) isFls f f'"
  unfolding restr_def perOfSet_def proof safe
    ...
    apply(cases "R 0 0")
      ...
      subgoal by simp (smt (verit, best) neper_per per_def) .        
    show "isFls f" by fact
    show "isFls f'" by fact
  qed
next
  fix g  
  assume "restr (rel_fun (=) R) (isFls_rlt R z) g g"
  hence "isFls_rlt R z g" and g: "rel_fun (=) R g g"
  unfolding restr_def by auto
  hence fin: "finite {n. n \<le> 0 \<and> \<not> R (g n) z}"   
  using R z by simp 
  hence ex: "\<exists>n. n > 0 \<or> R (g n) z" 
  	using infinite_nat_iff_unbounded_le  
  	using zero_less_one by blast

  define i0 where "i0 = (SOME i. i > 0 \<or> R (g i) z)"
  have Ri0: "i0 > 0 \<or> R (g i0) z"  
  	by (metis ex i0_def tfl_some)

  have fin': "finite (insert i0 {n. n \<le> 0 \<and> \<not> R (g n) z})"
   using fin by force
  ...
  show "\<exists>f. restr (rel_fun (=) (addp R z)) isFls f f \<and> 
            restr (rel_fun (=) R) (isFls_rlt R z) g (gg R z f)"  
  proof(rule exI[of _ f], unfold restr_def, safe)
    have "{n. n \<le> 0 \<and> f n \<noteq> 0} \<subseteq> insert i0 {n. n \<le> 0 \<and> \<not> R (g n) z}"   
    unfolding f_def apply safe apply(cases "R 0 0") by auto
    thus f: "isFls f" "isFls f" unfolding isFls_def2  
    	using fin' rev_finite_subset by blast+
    show "rel_fun (=) R g (gg R z f)"
    unfolding rel_fun_def gg_def f_def    
    by simp (smt (verit, best) R g neper_def per_def rel_funE z) 

    show "isFls_rlt R z g" by fact

    show f': "rel_fun (=) (addp R z) f f"
    using R z g unfolding rel_fun_def f_def addp_def by auto

    show "isFls_rlt R z (gg R z f)"
    using isFls_rlt_gg[OF R z f(1) f'] .  
  qed
qed 

(* *)

wide_typedef fls rel: rel_fls rep: "\<lambda>R1 R2. gg R1 R2 \<circ> fls_nth"
  subgoal using rel_fls_neper .
  subgoal using rel_fls_eq .
  subgoal using bij_upto_transportFromRaw[OF fls.type_definition_fls rel_fls_def, 
     unfolded isFls_def[abs_def, symmetric], OF bij_upto_gg] 
  unfolding isFls_rlt_def . 
