...
imports Complex_MainRLT Extended_Nat Liminf_Limsup
begin

text \<open>
  This should be part of \<^theory>\<open>HOLRLT-Library.Extended_Nat\<close> or \<^theory>\<open>HOLRLT-Library.Order_Continuity\<close>, but then the AFP-entry \<open>Jinja_Thread\<close> fails, as it does overload
  certain named from \<^theory>\<open>Complex_MainRLT\<close>.
\<close>
