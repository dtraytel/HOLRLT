...
theory Uprod imports "MainRLT" begin
...
(**********************)
(* The commute type has a wide definition *)

(* *)
definition isCommute :: "('a \<Rightarrow> 'a \<Rightarrow> 'b) \<Rightarrow> bool" where 
"isCommute f = (\<forall>x y. f x y = f y x)"

lemmas type_definition_isCommute = 
   commute.type_definition_commute[unfolded isCommute_def[symmetric]]

(* *)
lemma isCommute_aux: "isCommute = isCommute" ..
local_setup \<open>RLTHM @{binding isCommute_rlt_neper_param} @{thm isCommute_aux}\<close>
thm isCommute_rlt_neper_param

lemma isCommute_rlt_simp[simp]: 
"neper R1 \<Longrightarrow> neper R2 \<Longrightarrow> 
 isCommute_rlt R1 R2 f = (\<forall>a b. R1 a a \<and> R1 b b \<longrightarrow> R2 (f a b) (f b a))"
unfolding isCommute_rlt_def by auto

(* *)

definition gg :: "('a \<Rightarrow> 'a \<Rightarrow> bool) \<Rightarrow> ('b \<Rightarrow> 'b \<Rightarrow> bool) \<Rightarrow> ('a \<Rightarrow> 'a \<Rightarrow> 'b) \<Rightarrow> 'a \<Rightarrow> 'a \<Rightarrow> 'b" 
where "gg R1 R2 \<equiv> \<lambda>f a b. getRepr' R2 (f (getRepr' R1 a) (getRepr' R1 b))"

lemma bij_upto_gg: 
assumes R: "neper R1" "neper R2"
shows 
"bij_upto
 (restr (rel_fun R1 (rel_fun R1 R2)) isCommute)
 (restr (rel_fun R1 (rel_fun R1 R2)) (isCommute_rlt R1 R2)) (gg R1 R2)"
apply(rule bij_uptoI)
  subgoal apply(rule neper_restr)
    subgoal using R by simp
    subgoal apply(rule exI[of _ "\<lambda>a b. SOME x. R2 x x"], safe)
      subgoal using R by (simp add: neper_Some)
  ...
  subgoal for f f' unfolding restr_def apply safe
    ...
    subgoal using R unfolding isCommute_def[of f]  
    unfolding gg_def rel_fun_def 
    by simp (metis (no_types, lifting) getRepr'_def getReprOn getReprOn_UNIV geterRepr_related 
      neper_Some neper_getRepr_eq)
    subgoal using R unfolding isCommute_def[of f']  
    unfolding gg_def rel_fun_def 
    by simp (metis (no_types, lifting) getRepr'_def getReprOn getReprOn_UNIV geterRepr_related 
      neper_Some neper_getRepr_eq) .
  ...
  subgoal for g apply(rule exI[of _ "gg R1 R2 g"], unfold restr_def, safe)
     subgoal using R unfolding isCommute_def gg_def restr_def rel_fun_def 
     by (smt (verit, best) getRepr'_def getRepr_neper neper_getRepr_eq neper_per per_def) 
     subgoal using R unfolding isCommute_def gg_def 
     	 by simp (smt (verit, best) getRepr'_def getReprOn getReprOn_UNIV geterRepr_related 
         neper_Some neper_getRepr_eq rel_funE)
     subgoal using R unfolding isCommute_def gg_def 
     	 by simp (smt (verit, best) getRepr'_def getReprOn getReprOn_UNIV geterRepr_related 
         neper_Some neper_getRepr_eq rel_funE)
     subgoal using R unfolding rel_fun_def gg_def  
        ...
        neper_per per_def)
     subgoal using R unfolding gg_def  
     	 by simp (smt (verit, ccfv_SIG) getRepr'_def getRepr_neper neper_Some 
        neper_getRepr_eq rel_funE) . .


lemma rel_commute_alt: 
"rel_commute R1 R2 = inv_imagep (rel_fun R1 (rel_fun R1 R2)) apply_commute"
unfolding rel_commute_def inv_imagep_def restr_def fun_eq_iff by auto

thm bij_upto_transportFromRaw[OF type_definition_isCommute rel_commute_alt bij_upto_gg]

wide_typedef commute rel: rel_commute rep: "\<lambda>R1 R2. gg R1 R2 \<circ> apply_commute"
  subgoal for R1 R2 unfolding rel_commute_alt 
  apply(rule neper_inv_imagep[of _ _ "Abs_commute (\<lambda>a b. SOME x. R2 x x)"])
     subgoal by simp
     subgoal unfolding rel_fun_def  
     by auto (smt (verit, best) Abs_commute_inverse mem_Collect_eq neper_def someI_ex) .
  subgoal unfolding rel_commute_alt unfolding inv_imagep_def fun_eq_iff  
    by (simp add: apply_commute_inject)
  subgoal using bij_upto_transportFromRaw[OF type_definition_isCommute rel_commute_alt bij_upto_gg]
    unfolding isCommute_rlt_def .
