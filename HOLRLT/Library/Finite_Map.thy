...
(****************)
(* fmap has a wide type definition *)

declare option.rrel_eq[simp]

(* *)

lemma rel_fun_rrel_option_closed: 
"neper R1 \<Longrightarrow> neper R2 \<Longrightarrow> rel_fun R1 (rrel_option R2) f f \<Longrightarrow> 
 closed R1 {x. R1 x x \<and> f x \<noteq> None}"
unfolding rrel_alt 
unfolding rel_fun_def closed_def apply auto 
	apply (metis neper_def per_def)
  by (metis option_rel_Some1)

(* *)
definition "isFinDom \<equiv> \<lambda>m. finite (dom m)"

lemma fmap_type_definition_isFinDom: 
"type_definition fmlookup Abs_fmap (Collect isFinDom)"
using fmap.type_definition_fmap unfolding isFinDom_def by auto

lemma isFinDom_empty: "isFinDom Map.empty"
unfolding isFinDom_def by auto

(* *)
lemma isFinDom_aux: "isFinDom = isFinDom" ..
local_setup \<open>RLTHM @{binding isFinDom_rlt_aux} @{thm isFinDom_aux}\<close>

lemma isDom_rlt_simp[simp]:
"neper R1 \<Longrightarrow> neper R2 \<Longrightarrow> dom_rlt R2 R1 f = {x. R2 x x \<and> f x \<noteq> None}"
unfolding dom_rlt_def apply simp
unfolding rrel_alt rel_option_None2
unfolding rrel_option.simps by (auto simp: rlt_param)

lemma isFinDom_rlt_simp':
"neper R1 \<Longrightarrow> neper R2 \<Longrightarrow> 
 isFinDom_rlt R1 R2 f \<longleftrightarrow> finite_rlt R1 {x. R1 x x \<and> f x \<noteq> None}"
unfolding isFinDom_rlt_def by simp

lemma isFinDom_rlt_simp[simp]:
"neper R1 \<Longrightarrow> neper R2 \<Longrightarrow> rel_fun R1 (rrel_option R2) f f \<Longrightarrow> 
 isFinDom_rlt R1 R2 f \<longleftrightarrow> finite {getRepr R1 x | x. R1 x x \<and> f x \<noteq> None}"
apply(subst isFinDom_rlt_simp')
  apply auto[] apply auto[] 
  apply(subst finite_rlt_simp) 
  unfolding rel_set_def  rrel_alt
  apply auto
  apply (smt (verit, best) CollectD CollectI closed_def neper_classes_eq option.rel_cases rel_funE)
  done
(* *)

definition "rrel_fun R1 R2 f g \<equiv> rel_fun R1 R2 (f o getRepr R1) (g o getRepr R1)"

lemma rrel_fun_neper[simp]: "neper R1 \<Longrightarrow> neper R2 \<Longrightarrow> neper (rrel_fun R1 R2)"
unfolding rrel_fun_def neper_def per_def apply safe
  apply (smt (verit, best) rel_fun_def)
  apply (smt (verit, ccfv_SIG) rel_fun_def)
  subgoal for a b apply(rule exI[of _ "\<lambda>_. b"])
  unfolding rel_fun_def comp_def by blast .

lemma rrel_fun[simp]: "rrel_fun (=) (=) = (=)"
unfolding rrel_fun_def by auto

definition rrel_fmap :: 
"('a \<Rightarrow> 'a \<Rightarrow> bool) \<Rightarrow> ('b \<Rightarrow> 'b \<Rightarrow> bool) \<Rightarrow> ('a, 'b) fmap \<Rightarrow> ('a, 'b) fmap \<Rightarrow> bool" 
where 
"rrel_fmap R1 R2 = inv_imagep (rrel_fun R1 (rrel_option R2)) fmlookup"

lemma rrel_fmap_neper[simp]: "neper R1 \<Longrightarrow> neper R2 \<Longrightarrow> neper (rrel_fmap R1 R2)"
unfolding rrel_fmap_def apply(rule neper_inv_imagep[of _ fmlookup fmempty])
  subgoal by (simp add: option.rrel_neper)
  subgoal unfolding rrel_fun_def comp_def rel_fun_def 
  	by (auto simp: rrel_alt) .

lemma rrel_fmap_eq[simp]: "rrel_fmap (=) (=) = (=)"
unfolding rrel_fmap_def inv_imagep_def fun_eq_iff 
	by (simp add: fmlookup_inject)

(* Compatibility with the standard one-argument relator *)
lemma fmrel_rrel_fmap: 
assumes R: "neper R" 
shows "fmrel R g1 g2 \<longleftrightarrow> rrel_fmap (=) R g1 g2"
unfolding fmrel_def rrel_fmap_def BNF_Def.vimage2p_def inv_imagep_def
rrel_fun_def rel_fun_def rrel_alt by auto

(* *)

definition gg :: 
"('a \<Rightarrow> 'a \<Rightarrow> bool) \<Rightarrow> ('b \<Rightarrow> 'b \<Rightarrow> bool) \<Rightarrow> ('a \<Rightarrow> 'b option) \<Rightarrow> 'a \<Rightarrow> 'b option" where 
"gg \<equiv> \<lambda>R1 R2 f. f o getRepr R1"

(* *)

lemma rel_fun_isFinDom_rlt_gg: 
assumes R1: "neper R1" and R2: "neper R2" and f: "isFinDom f" 
and f1: "rel_fun R1 (rrel_option R2) (f o getRepr R1) (f o getRepr R1)"
shows "rel_fun R1 (rrel_option R2) (gg R1 R2 f) (gg R1 R2 f) \<and> isFinDom_rlt R1 R2 (gg R1 R2 f)"
proof safe
   have g: "\<And>x y. R1 x y \<Longrightarrow> getRepr R1 x = getRepr R1 y"
  by (simp add: R1 neper_getRepr_eq) 
  show 0: "rel_fun R1 (rrel_option R2) (gg R1 R2 f) (gg R1 R2 f)" 
  using f1 unfolding rel_fun_def rrel_alt gg_def
  using g by auto  
  have "finite {getRepr R1 x |x. R1 x x \<and> gg R1 R2 f x \<noteq> None}" 
  using f unfolding isFinDom_def gg_def dom_def  
   by auto (smt (verit, best) Collect_mono rev_finite_subset)
  thus "isFinDom_rlt R1 R2 (gg R1 R2 f)"
  using 0 R1 R2 by simp
qed

lemmas rel_fun_rlt_gg = rel_fun_isFinDom_rlt_gg[THEN conjunct1]
lemmas isFinDom_rlt_gg = rel_fun_isFinDom_rlt_gg[THEN conjunct2]

lemma gg_eq[simp]: "gg (=) (=) = id"
...
shows 
"bij_upto
  (restr (rrel_fun R1 (rrel_option R2)) 
         isFinDom)
  (restr (rel_fun R1 (rrel_option R2)) 
         (isFinDom_rlt R1 R2)) 
  (gg R1 R2)"
proof(rule bij_uptoI) 
  show "neper (restr (rel_fun R1 (rrel_option R2)) (isFinDom_rlt R1 R2))"
  apply(rule neper_restr)
    subgoal by (simp add: R option.rrel_neper)
    subgoal apply(rule exI[of _ "\<lambda>_. None"])  
    using R by (simp add: rel_fun_def rrel_alt) .
next
  fix f f' 
  assume "restr (rrel_fun R1 (rrel_option R2)) isFinDom f f'"
  hence ff': "rel_fun R1 (rrel_option R2) (f o getRepr R1) (f' o getRepr R1)" "isFinDom f" "isFinDom f'" 
  unfolding restr_def rrel_fun_def by auto
  show "restr (rel_fun R1 (rrel_option R2)) (isFinDom_rlt R1 R2) (gg R1 R2 f) (gg R1 R2 f')" 
  unfolding restr_def proof safe
    show "rel_fun R1 (rrel_option R2) (gg R1 R2 f) (gg R1 R2 f')" 
    using ff'(1) unfolding rel_fun_def gg_def by auto
  next
    show "isFinDom_rlt R1 R2 (gg R1 R2 f)" 
    	using R ff'(2) isFinDom_rlt_gg  
    	by (metis (mono_tags, lifting) gg_def isFinDom_rlt_simp' neper_eq option.rrel_eq rel_fun_getRepr_eq)
  next
    show "isFinDom_rlt R1 R2 (gg R1 R2 f')" 
    	using R ff'(3) isFinDom_rlt_gg 
      by (metis (mono_tags, lifting) gg_def isFinDom_rlt_simp' neper_eq option.rrel_eq rel_fun_getRepr_eq)
  qed
next
  fix f f'
  assume "restr (rrel_fun R1 (rrel_option R2)) isFinDom f f"
  "restr (rrel_fun R1 (rrel_option R2)) isFinDom f' f'"
  "restr (rel_fun R1 (rrel_option R2)) (isFinDom_rlt R1 R2) (gg R1 R2 f) (gg R1 R2 f')"

  hence ff: "isFinDom f" "isFinDom f'"  
  "rrel_fun R1 (rrel_option R2) f f" "rrel_fun R1 (rrel_option R2) f' f'"
  and gg: "isFinDom_rlt R1 R2 (gg R1 R2 f)" "isFinDom_rlt R1 R2 (gg R1 R2 f')"
  "rel_fun R1 (rrel_option R2) (gg R1 R2 f) (gg R1 R2 f')" 
  unfolding restr_def by auto 

  have ff1: "rel_fun R1 (rrel_option R2) (f \<circ> getRepr R1) (f \<circ> getRepr R1)"
       "rel_fun R1 (rrel_option R2) (f' \<circ> getRepr R1) (f' \<circ> getRepr R1)"
  using ff unfolding rel_fun_def  rrel_fun_def
  by auto  

  show "restr (rrel_fun R1 (rrel_option R2)) isFinDom f f'"
  unfolding restr_def rrel_fun_def proof safe
    show "rel_fun R1 (rrel_option R2) (f \<circ> getRepr R1) (f' \<circ> getRepr R1)"
    using ff(3,4) gg(3) unfolding rel_fun_def using ff1  unfolding gg_def by auto
    show "isFinDom f" by fact
    show "isFinDom f'" by fact
  qed
next
  fix g  
  assume "restr (rel_fun R1 (rrel_option R2)) (isFinDom_rlt R1 R2) g g"
  hence "isFinDom_rlt R1 R2 g" and g: "rel_fun R1 (rrel_option R2) g g"
  unfolding restr_def by auto
  hence fin: "finite {getRepr R1 x |x. R1 x x \<and> g x \<noteq> None}" 
  using R by simp thm isFinDom_rlt_simp
  note g = g[unfolded rrel_alt]

  define f where "f \<equiv> \<lambda>x. if R1 x x \<and> x = getRepr R1 x then g x else None" 

  show "\<exists>f. restr (rrel_fun R1 (rrel_option R2)) isFinDom f f \<and>
            restr (rel_fun R1 (rrel_option R2)) (isFinDom_rlt R1 R2) g (gg R1 R2 f)"
  proof(rule exI[of _ f], unfold restr_def rrel_fun_def, safe)
    have "{a. f a \<noteq> None} \<subseteq> {getRepr R1 x |x. R1 x x \<and> g x \<noteq> None}"
    unfolding f_def apply safe subgoal for x y apply(rule exI[of _ x])
    apply(cases "R1 x x") using g
    by auto (meson option.distinct(1))+ .
    thus f: "isFinDom f" "isFinDom f" unfolding isFinDom_def dom_def  
    	using fin infinite_super by blast+
    show "rel_fun R1 (rrel_option R2) g (gg R1 R2 f)"
    unfolding rel_fun_def gg_def rrel_alt apply auto
    unfolding f_def apply auto   
    apply (metis (no_types, lifting) R(1) apply_rsp' g getRepr_neper neper_per per_def) 
    apply (metis (no_types, lifting) R(1) getRepr_neper neper_def per_def)     
    apply (metis CollectD CollectI R(1) geterRepr_related neper_classes_eq neper_getRepr_eq) .
    show " isFinDom_rlt R1 R2 g" by fact
    show "isFinDom_rlt R1 R2 (gg R1 R2 f)" 
    using isFinDom_rlt_gg[OF R f(1)]  
    by (metis \<open>rel_fun R1 (rrel_option R2) g (gg R1 R2 f)\<close> R 
      gg_def mem_Collect_eq neper_classes_eq neper_rel_fun option.rrel_neper) 
  next
    show "rel_fun R1 (rrel_option R2) (f \<circ> getRepr R1) (f \<circ> getRepr R1)" 
    by (smt (verit, best) R comp_apply f_def g neper_getRepr_eq option.ctr_transfer(1) 
      rel_funE rel_funI rrel_alt)
  qed
qed 

(* *)

wide_typedef fmap rel: rrel_fmap rep: "\<lambda>R1 R2. gg R1 R2 \<circ> fmlookup"
  subgoal using rrel_fmap_neper .
  subgoal using rrel_fmap_eq .
  subgoal using bij_upto_transportFromRaw[OF fmap.type_definition_fmap rrel_fmap_def, 
     unfolded isFinDom_def[abs_def, symmetric], OF bij_upto_gg] 
  unfolding isFinDom_rlt_def . 
