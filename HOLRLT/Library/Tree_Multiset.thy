...
text \<open>
  Kept separate from theory \<^theory>\<open>HOLRLT-Library.Tree\<close> to avoid importing all of theory \<^theory>\<open>HOLRLT-Library.Multiset\<close> into \<^theory>\<open>HOLRLT-Library.Tree\<close>. Should be merged if \<^theory>\<open>HOLRLT-Library.Multiset\<close> ever becomes part of \<^theory>\<open>MainRLT\<close>.
\<close>
