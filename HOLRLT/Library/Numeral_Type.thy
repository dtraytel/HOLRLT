...
(*******************)
(* bit0 and bit1 have wide type definitions *)

(* Preliminaries *)
(* *)
definition surjf :: "int \<Rightarrow> int \<Rightarrow> int \<Rightarrow> int" where 
"surjf i j \<equiv> \<lambda>u. if u < j then u else j-1"

lemma surjf_between: "0 < j \<Longrightarrow> j \<le> i \<Longrightarrow> 0 \<le> u \<Longrightarrow> u < i \<Longrightarrow> 
  0 \<le> surjf i j u \<and> surjf i j u < j"
unfolding surjf_def by fastforce

lemma surjf_surj: "0 < j \<Longrightarrow> j \<le> i \<Longrightarrow> 
  0 \<le> u \<Longrightarrow> u < j \<Longrightarrow> 
  \<exists>v. (0 \<le> v \<and> v < i) \<and> u = surjf i j v"
unfolding surjf_def by fastforce

lemma surj_surjf: "0 < j \<Longrightarrow> j \<le> i \<Longrightarrow> surjf i j ` {0..<i} = {0..<j}"
unfolding surjf_def by fastforce

term "atLeastLessThan 0 (int (card(UNIV::('a::finite)set)))"

(* bit0 has a wide type definition *)

definition isBit0 where 
"isBit0 (a::'a::finite) i \<equiv> 0 \<le> i \<and> i < 2 * int CARD('a::finite)"

lemma atLeastLessThan_Collect_bit0: 
"{0 ..< 2 * int CARD('a::finite)} = Collect (isBit0 (a::'a))"
unfolding isBit0_def by auto

lemma bit0_type_definition: 
"type_definition 
   (Rep_bit0::'a bit0 \<Rightarrow> int) Abs_bit0 
   (Collect (isBit0 (a::'a::finite)))"
unfolding atLeastLessThan_Collect_bit0[symmetric]  
using type_definition_bit0 by blast

(* *)


lemma isBit0_aux: "isBit0 = isBit0" ..
local_setup \<open>RLTHM @{binding isBit0_rlt_neper_param} @{thm isBit0_aux}\<close>
thm isBit0_rlt_neper_param
(* A to D: This is the issue with itself, right? *)

lemma isBit0_rlt_simp[simp]: "neper (R::'a::finite\<Rightarrow>'a\<Rightarrow>bool) \<Longrightarrow> 
 isBit0_rlt R a = 
 (\<lambda>i. (0 \<le> i \<and> i < 2 * int (card {getRepr R a |a. R a a})))"
unfolding isBit0_rlt_def fun_eq_iff   
apply simp unfolding rlt_eq by simp

definition hh0 where 
"hh0 \<equiv> \<lambda>R::'a::finite\<Rightarrow>'a\<Rightarrow>bool. 
    surjf (2 * int (CARD('a))) 
          (2 * int (card {getRepr R a |a. R a a}))"

definition bit0_rel :: "('a::finite \<Rightarrow> 'a \<Rightarrow> bool) \<Rightarrow> 'a bit0 \<Rightarrow> 'a bit0 \<Rightarrow> bool" 
where "bit0_rel R \<equiv> 
inv_imagep 
  (restr (inv_imagep (restr (=) (isBit0_rlt R undefined)) (hh0 R)) 
         (isBit0 (undefined::'a))) 
  Rep_bit0"

lemma isBit0_rlt_hh0:
"neper (R::'a::finite\<Rightarrow>'a\<Rightarrow>bool) \<Longrightarrow> 
 isBit0 (undefined::'a) x \<Longrightarrow> isBit0_rlt R (undefined::'a) (hh0 R x)"
unfolding isBit0_def isBit0_rlt_simp  hh0_def 
  unfolding isBit0_def isBit0_rlt_simp hh0_def 
  apply(rule surjf_between) apply auto  
  using card_gt_0_iff ne_getRepr apply fastforce 
  by (simp add: card_mono)

lemma bij_upto_hh0: 
assumes R: "neper (R::'a::finite\<Rightarrow>'a\<Rightarrow>bool)"
shows "bij_upto (bit0_rel R) (restr (=) (isBit0_rlt R undefined)) (hh0 R \<circ> Rep_bit0::'a bit0 \<Rightarrow> int)"
apply(rule bij_upto_criterion[OF bit0_type_definition, of "(=)" "undefined::'a" "isBit0_rlt R undefined" "hh0 R",simped,
unfolded bit0_rel_def[symmetric]])
  subgoal using isBit0_rlt_hh0[OF R] . 
  subgoal for u unfolding isBit0_def isBit0_rlt_simp[OF R] hh0_def 
  apply(rule surjf_surj) by (auto simp: card_mono) .

lemma hh0_eq': "0 \<le> i \<Longrightarrow> i < 2 * int CARD('a::finite) \<Longrightarrow> hh0 ((=)::'a\<Rightarrow>'a\<Rightarrow>bool) i = i"
unfolding hh0_def surjf_def by auto

lemma hh0_eq[simp]: "hh0 ((=)::'a::finite\<Rightarrow>'a\<Rightarrow>bool) (Rep_bit0 (a::'a bit0)) = Rep_bit0 a"
apply(rule hh0_eq') using Rep_bit0 by auto

lemma bit0_rel_neper: 
"neper (R::'a::finite\<Rightarrow>'a\<Rightarrow>bool) \<Longrightarrow> neper (bit0_rel R::'a bit0 \<Rightarrow> 'a bit0 \<Rightarrow> bool)"
unfolding bit0_rel_def
apply(rule neper_criterion[OF bit0_type_definition, of _ "undefined::'a"]) 
  subgoal by auto
  subgoal apply(rule exI[of _ 0]) unfolding isBit0_def by auto
  subgoal apply(drule isBit0_rlt_hh0) by auto .

lemma bit0_rel_eq[simp]: "bit0_rel (((=)::'a::finite\<Rightarrow>'a\<Rightarrow>bool)) = (=)"
unfolding bit0_rel_def rlt_eq inv_imagep_def fun_eq_iff restr_def apply simp
using Rep_bit0 Rep_bit0_inject atLeastLessThan_Collect_bit0 by blast 

(* *)

wide_typedef bit0 rel: bit0_rel rep: "\<lambda>R. hh0 R \<circ> Rep_bit0"
  subgoal using bit0_rel_neper .
  subgoal using bit0_rel_eq .
  subgoal for R 
  unfolding rlt_eq apply simp  
  unfolding isBit0_rlt_simp[of R undefined, symmetric]
  using bij_upto_hh0 .
  subgoal unfolding fun_eq_iff comp_def apply safe using hh0_eq . .

(*****)
(* bit1 has a wide type definition *)

definition isBit1 where 
"isBit1 (a::'a::finite) i \<equiv> 0 \<le> i \<and> i < 1 + 2 * int CARD('a::finite)"

lemma atLeastLessThan_Collect_bit1: 
"{0 ..< 1 + 2 * int CARD('a::finite)} = Collect (isBit1 (a::'a))"
unfolding isBit1_def by auto

lemma bit1_type_definition: 
"type_definition 
   (Rep_bit1::'a bit1 \<Rightarrow> int) Abs_bit1 
   (Collect (isBit1 (a::'a::finite)))"
unfolding atLeastLessThan_Collect_bit1[symmetric]  
using type_definition_bit1 by blast

(* *)

lemma isBit1_aux: "isBit1 = isBit1" ..
local_setup \<open>RLTHM @{binding isBit1_rlt_neper_param} @{thm isBit1_aux}\<close>
thm isBit1_rlt_neper_param

lemma isBit1_rlt_simp[simp]: "neper (R::'a::finite\<Rightarrow>'a\<Rightarrow>bool) \<Longrightarrow> 
 isBit1_rlt R a = 
 (\<lambda>i. (0 \<le> i \<and> i < 1 + 2 * int (card {getRepr R a |a. R a a})))"
unfolding isBit1_rlt_def fun_eq_iff   
apply simp unfolding rlt_eq by simp

definition hh1 where 
"hh1 \<equiv> \<lambda>R::'a::finite\<Rightarrow>'a\<Rightarrow>bool. 
    surjf (1 + 2 * int (CARD('a))) 
          (1 + 2 * int (card {getRepr R a |a. R a a}))"

definition bit1_rel :: "('a::finite \<Rightarrow> 'a \<Rightarrow> bool) \<Rightarrow> 'a bit1 \<Rightarrow> 'a bit1 \<Rightarrow> bool" 
where "bit1_rel R \<equiv> 
inv_imagep 
  (restr (inv_imagep (restr (=) (isBit1_rlt R undefined)) (hh1 R)) 
         (isBit1 (undefined::'a))) 
  Rep_bit1"

lemma isBit1_rlt_hh1:
"neper (R::'a::finite\<Rightarrow>'a\<Rightarrow>bool) \<Longrightarrow> 
 isBit1 (undefined::'a) x \<Longrightarrow> isBit1_rlt R (undefined::'a) (hh1 R x)"
unfolding isBit1_def isBit1_rlt_simp  hh1_def 
  unfolding isBit1_def isBit1_rlt_simp hh1_def 
  apply(rule surjf_between) apply auto  
  by (simp add: card_mono)
...
lemma bij_upto_hh1: 
assumes R: "neper (R::'a::finite\<Rightarrow>'a\<Rightarrow>bool)"
shows "bij_upto (bit1_rel R) (restr (=) (isBit1_rlt R undefined)) (hh1 R \<circ> Rep_bit1::'a bit1 \<Rightarrow> int)"
apply(rule bij_upto_criterion[OF bit1_type_definition, of "(=)" "undefined::'a" "isBit1_rlt R undefined" "hh1 R",simped,
unfolded bit1_rel_def[symmetric]])
  subgoal using isBit1_rlt_hh1[OF R] . 
  subgoal for u unfolding isBit1_def isBit1_rlt_simp[OF R] hh1_def 
  apply(rule surjf_surj) by (auto simp: card_mono) .

lemma hh1_eq': "0 \<le> i \<Longrightarrow> i < 1 + 2 * int CARD('a::finite) \<Longrightarrow> 
    hh1 ((=)::'a\<Rightarrow>'a\<Rightarrow>bool) i = i"
unfolding hh1_def surjf_def by auto

lemma hh1_eq[simp]: "hh1 ((=)::'a::finite\<Rightarrow>'a\<Rightarrow>bool) (Rep_bit1 (a::'a bit1)) = Rep_bit1 a"
apply(rule hh1_eq') using Rep_bit1 by auto

lemma bit1_rel_neper: 
"neper (R::'a::finite\<Rightarrow>'a\<Rightarrow>bool) \<Longrightarrow> neper (bit1_rel R::'a bit1 \<Rightarrow> 'a bit1 \<Rightarrow> bool)"
unfolding bit1_rel_def
apply(rule neper_criterion[OF bit1_type_definition, of _ "undefined::'a"]) 
  subgoal by auto
  subgoal apply(rule exI[of _ 0]) unfolding isBit1_def by auto
  subgoal apply(drule isBit1_rlt_hh1) by auto .

lemma bit1_rel_eq[simp]: "bit1_rel (((=)::'a::finite\<Rightarrow>'a\<Rightarrow>bool)) = (=)"
unfolding bit1_rel_def rlt_eq inv_imagep_def fun_eq_iff restr_def apply simp
using Rep_bit1 Rep_bit1_inject atLeastLessThan_Collect_bit1 by blast 

(* *)

wide_typedef bit1 rel: bit1_rel rep: "\<lambda>R. hh1 R \<circ> Rep_bit1"
  subgoal using bit1_rel_neper .
  subgoal using bit1_rel_eq .
  subgoal for R 
  unfolding rlt_eq apply simp  
  unfolding isBit1_rlt_simp[of R undefined, symmetric]
  using bij_upto_hh1 .
  subgoal unfolding fun_eq_iff comp_def apply safe using hh1_eq . .
