...
(*   *)
(**********************************************)
(* Relativization of the "countable" predicate  *)

thm countable_def
lemma countable_aux: "countable = countable" ..
local_setup \<open>RLTHM @{binding countable_rlt_aux} @{thm countable_aux}\<close>

find_theorems inj_on_rlt

lemma inj_on_rlt_simp[simp]: 
"neper P \<Longrightarrow> neper Q \<Longrightarrow> 
 inj_on_rlt P Q f A \<longleftrightarrow> 
 (\<forall>x1 x2 a1 a2. {a1,a2} \<subseteq> A \<and> P a1 x1 \<and> P a2 x2 \<and> Q (f x1) (f x2) 
                 \<longrightarrow> P x1 x2)"
unfolding inj_on_rlt_def by auto

lemma countable_rlt_simp[simp]: "neper R \<Longrightarrow> 
 countable_rlt R (A::'a set) \<longleftrightarrow> 
 (\<exists>f::'a\<Rightarrow>nat. 
    rel_fun R (=) f f \<and>
    (\<forall>x1 x2 a1 a2. {a1,a2} \<subseteq> A \<and> R a1 x1 \<and> R a2 x2 \<and> f x1 = f x2
                   \<longrightarrow> R x1 x2))"
unfolding countable_rlt_def by simp
