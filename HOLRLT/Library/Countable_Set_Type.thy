...
lemma neper_rel_cset[simp]: "neper R \<Longrightarrow> neper (rel_cset R)"
apply(rule relator_pres_neper[of rel_cset])
	subgoal using cset.rel_compp .
  subgoal using cset.rel_conversep .
  subgoal using cempty_parametric by blast
  subgoal . .

definition gg where "gg R A \<equiv> {x \<in> rcset A. R x x}"

(* It seems that these three lemmas, if they can be proved for a 
typedef predicate and its relativization, can ensure wideness: *)
lemma neper_countable_rlt: "neper R \<Longrightarrow> countable_rlt R {}"
apply simp unfolding rel_fun_def by auto

lemma countable_imp_countable_rlt:   
assumes R: "neper R" and "countable A" and A: "rel_set R A A"
shows "countable_rlt R {x \<in> A. R x x}"
using assms unfolding countable_def apply safe
 subgoal for f apply simp
 apply(rule exI[of _ "f o getReprOn A R"])
 apply auto  
 	 apply (simp add: rel_fun_getReprOn_eq) 
   unfolding inj_on_def 
   by (smt (verit) getReprOn_in neper_def per_def) .
    
lemma countable_rlt_imp_countable_getRepr:
assumes R: "neper R" and rA: "countable_rlt R A" and RA: "rel_set R A A"
shows "countable {getRepr R a |a. a \<in> A}"
using assms unfolding countable_def apply clarsimp
subgoal for f apply(rule exI[of _ f]) unfolding inj_on_def apply auto
by (smt (verit, best) getRepr_inject getRepr_neper rel_setD2) .
 
lemma bij_upto_gg: 
assumes R: "neper R"
shows "bij_upto (rel_cset R) (restr (rel_set R) (countable_rlt R)) (gg R)"
proof(rule bij_uptoI)
  show "neper (restr (rel_set R) (countable_rlt R))" 
  by (meson assms empty_transfer neper_countable_rlt neper_restr set.rrel_neper)
next
  fix A1 A2
  assume RA12: "rel_cset R A1 A2"
  show "restr (rel_set R) (countable_rlt R) (gg R A1) (gg R A2)"
  unfolding restr_def proof safe
    show "rel_set R (gg R A1) (gg R A2)" 
    using RA12 unfolding gg_def rel_cset_def rel_set_def  
    by simp (metis assms neper_per per_def)
  next
    show "countable_rlt R (gg R A1)"
    unfolding gg_def apply(rule countable_imp_countable_rlt[OF R])
    by simp (metis RA12 assms neper_per per_def per_rel_set rel_cset.rep_eq)
  next
    show "countable_rlt R (gg R A2)"
    unfolding gg_def apply(rule countable_imp_countable_rlt[OF R])
    by simp (metis RA12 assms neper_per per_def per_rel_set rel_cset.rep_eq)
  qed
next
  fix A1 A2
  assume A1: "rel_cset R A1 A1" and A2: "rel_cset R A2 A2"
  and A12: "restr (rel_set R) (countable_rlt R) (gg R A1) (gg R A2)"
  have "rel_set R (rcset A1) (rcset A2)" 
  using A12 A1 A2 unfolding restr_def gg_def rel_set_def apply simp
  by (metis (no_types, lifting) assms neper_per per_def rel_cset_alt_def)
  thus "rel_cset R A1 A2" unfolding rel_cset_def by simp
next
  fix A assume "restr (rel_set R) (countable_rlt R) A A"
  hence A: "rel_set R A A" "countable_rlt R A" unfolding restr_def by auto
  define F where "F = acset {getRepr R a | a.  a \<in> A}"
  have [simp]: "rcset (acset {getRepr R a |a. a \<in> A}) = {getRepr R a |a. a \<in> A}"
  by (metis (mono_tags, lifting) A(1) A(2) acset_inverse assms 
    countable_rlt_imp_countable_getRepr mem_Collect_eq)
  have fF: "rcset F = {getRepr R a | a.  a \<in> A}" 
  by (metis (mono_tags, lifting) A(1) A(2) acset_inverse F_def assms 
    countable_rlt_imp_countable_getRepr mem_Collect_eq)

  show "\<exists>F. rel_cset R F F \<and> restr (rel_set R) (countable_rlt R) A (gg R F)"
  proof(rule exI[of _ F], unfold restr_def, safe)
    show RFF: "rel_cset R F F"
    unfolding F_def rel_cset_def 
    ...
    show "countable_rlt R A" by fact

    show "countable_rlt R (gg R F)" 
    unfolding gg_def apply(rule countable_imp_countable_rlt[OF R])
    using RFF rel_cset.rep_eq by auto
  qed
qed

lemma gg_eq[simp]: "gg (=) = rcset"
unfolding gg_def by auto

wide_typedef cset rel: rel_cset rep: gg
  subgoal using neper_rel_cset .
  subgoal using cset.rel_eq .
