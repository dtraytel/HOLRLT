...
(* the type "node" has a wide definition *)

(* *)
definition "isNode \<equiv> \<lambda>p. \<exists>f x k. p = (f :: nat => 'b + nat, x ::'a + nat) \<and> f k = Inr 0"

lemma isNode_alt: "isNode p \<longleftrightarrow> (\<exists>k. fst p k = Inr 0)"
unfolding isNode_def by (metis eq_fst_iff)

find_theorems name: node name: type_def
lemmas type_definition_isNode = node.type_definition_node[unfolded Node_def isNode_def[symmetric]]

(* *)
lemma isNode_aux: "isNode = isNode" ..
local_setup \<open>RLTHM @{binding isNode_rlt_neper_param} @{thm isNode_aux}\<close>
thm isNode_rlt_neper_param

(* *)
local_setup \<open>RLTHM @{binding isNode_rlt_alt} @{thm isNode_alt}\<close>
thm isNode_rlt_alt

(* *)
lemma isNode_Node: "isNode = (\<lambda>x. x \<in> Old_Datatype.Node)" 
unfolding isNode_def Old_Datatype.Node_def by auto
local_setup \<open>RLTHM @{binding isNode_rlt_Node_rlt} @{thm isNode_Node}\<close>
thm isNode_rlt_Node_rlt
(* *)


(* *)
lemma isNode_rlt_simp: 
"neper R1 \<Longrightarrow> neper R2 \<Longrightarrow> rel_prod (rel_fun (=) (rel_sum R1 (=))) (rel_sum R2 (=)) p p  
\<Longrightarrow> isNode_rlt R1 R2 p = isNode p"
unfolding isNode_rlt_alt fun_eq_iff isNode_alt apply (simp add: sum.rrel_neper rlt_eq rlt_param) 
by (smt (verit, best) rel_sum.cases rel_sum_simps(4) sum.simps(4))
...
shows 
"bij_upto
 (restr (rel_prod (rel_fun (=) (rel_sum R1 (=))) (rel_sum R2 (=))) isNode)
 (restr (rel_prod (rel_fun (=) (rel_sum R1 (=))) (rel_sum R2 (=))) (isNode_rlt R1 R2)) id"
apply(rule bij_upto_id')
  subgoal apply(rule neper_restr)
    subgoal by (simp add: R prod.rrel_neper sum.rrel_neper)
    subgoal apply(rule exI[of _ "(\<lambda>x. Inr 0, Inr 0)"])
    unfolding isNode_alt by auto .
  subgoal unfolding restr_def fun_eq_iff apply(intro allI antisym)
    subgoal for x xa apply(rule le_boolI) apply(subst isNode_rlt_simp[OF R])
      subgoal using R 
        apply(subgoal_tac "neper (rel_prod (rel_fun (=) (rel_sum R1 (=))) (rel_sum R2 (=)))")
        unfolding neper_def per_def apply blast 
        unfolding neper_def[symmetric] per_def[symmetric]  
        by (simp add: prod.rrel_neper sum.rrel_neper) 
      subgoal apply(subst isNode_rlt_simp[OF R])
        subgoal using R apply(subgoal_tac "neper (rel_prod (rel_fun (=) (rel_sum R1 (=))) (rel_sum R2 (=)))") 
          unfolding neper_def per_def apply blast 
          unfolding neper_def[symmetric] per_def[symmetric]  
          by (simp add: prod.rrel_neper sum.rrel_neper) 
        subgoal by simp . .
    subgoal for x xa apply(rule le_boolI) apply(subst isNode_rlt_simp[OF R, symmetric])
      subgoal using R 
        apply(subgoal_tac "neper (rel_prod (rel_fun (=) (rel_sum R1 (=))) (rel_sum R2 (=)))")
        unfolding neper_def per_def apply blast 
        unfolding neper_def[symmetric] per_def[symmetric]  
        by (simp add: prod.rrel_neper sum.rrel_neper) 
      subgoal apply(subst isNode_rlt_simp[OF R, symmetric])
        subgoal using R apply(subgoal_tac "neper (rel_prod (rel_fun (=) (rel_sum R1 (=))) (rel_sum R2 (=)))") 
          unfolding neper_def per_def apply blast 
          unfolding neper_def[symmetric] per_def[symmetric]  
          by (simp add: prod.rrel_neper sum.rrel_neper) 
...
definition rel_node where 
"rel_node R1 R2 = inv_imagep (rel_prod (rel_fun (=) (rel_sum R2 (=))) (rel_sum R1 (=))) Rep_Node" 

thm bij_upto_transportFromRaw[OF type_definition_isNode rel_node_def bij_upto_node]

thm isNode_rlt_Node_rlt 

lemma node_auxx: 
assumes R: "neper R1" "neper R2"
and p: "rel_prod (rel_fun (=) (rel_sum R1 (=))) (rel_sum R2 (=)) p p"
shows "isNode_rlt R1 R2 p \<longleftrightarrow> 
       member_rlt (rel_prod (rel_fun (=) (rel_sum R1 (=))) (rel_sum R2 (=))) 
         p 
         (Node_rlt R1 R2)"
unfolding isNode_rlt_Node_rlt[OF R(2,1), unfolded rel_fun_def, rule_format, OF 
p[unfolded rel_fun_def]] using R by (simp add: rel_fun_eq_rel)

lemma node_aux: 
assumes R: "neper R1" "neper R2"
shows 
"(restr (rel_prod (rel_fun (=) (rel_sum R1 (=))) (rel_sum R2 (=)))
   (\<lambda>x. member_rlt (rel_prod (rel_fun (=) (rel_sum R1 (=))) (rel_sum R2 (=))) x (Node_rlt R1 R2)))
=
(restr (rel_prod (rel_fun (=) (rel_sum R1 (=))) (rel_sum R2 (=))) 
   (isNode_rlt R1 R2))
"
unfolding restr_def fun_eq_iff using node_auxx[OF R] 
apply(intro allI conjI antisym le_boolI)
  subgoal by simp
  subgoal apply(subst node_auxx[OF R]) 
  by auto (metis R mem_Collect_eq neper_classes_eq neper_eq neper_rel_fun prod.rrel_neper 
    sum.rrel_neper)
  subgoal apply(subst node_auxx[OF R]) 
  by auto (metis R mem_Collect_eq neper_classes_eq neper_eq neper_rel_fun prod.rrel_neper 
    sum.rrel_neper)
  subgoal by simp
  subgoal apply(subst node_auxx[OF R, symmetric]) apply auto  
  apply(subgoal_tac "neper (rel_prod (rel_fun (=) (rel_sum R1 (=))) (rel_sum R2 (=)))")
    unfolding neper_def per_def apply blast
    unfolding neper_def[symmetric] per_def[symmetric]   
    using R by (simp add: prod.rrel_neper sum.rrel_neper) 
  subgoal apply(subst node_auxx[OF R, symmetric]) apply auto  
  apply(subgoal_tac "neper (rel_prod (rel_fun (=) (rel_sum R1 (=))) (rel_sum R2 (=)))")
    unfolding neper_def per_def apply blast
    unfolding neper_def[symmetric] per_def[symmetric]   
...
wide_typedef Old_Datatype.node rel: rel_node rep: "\<lambda>R1 R2. Rep_Node"
  subgoal for R1 R2 unfolding rel_node_def
  apply(rule neper_inv_imagep[of _ _ "Abs_Node (\<lambda>x. Inr 0, Inr 0)"])
     subgoal by (simp add: prod.rrel_neper sum.rrel_neper)
     subgoal unfolding rel_fun_def by (simp add: Abs_Node_inverse Node_K0_I) .
  subgoal unfolding rel_node_def unfolding inv_imagep_def fun_eq_iff  
    by (simp add: Rep_Node_inject prod.rel_eq sum.rel_eq)
  subgoal for R1 R2 using bij_upto_transportFromRaw[OF type_definition_isNode rel_node_def bij_upto_node,simped]
     unfolding node_aux[of] .
  subgoal .. .
